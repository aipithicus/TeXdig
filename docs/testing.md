...
The ordinary test command runs the default conformance tier: every explicit family, the 346,200 UTF-8 class sequences through length four, the seeded UTF-8 random census, the 500-case occurrence lookup census, the 500-case occurrence-selection Boolean census, and the 500-case SpanSet bitmap census. `conformance:check` regenerates default-tier fixture content and validates the retained deep digest header and count without recomputing its value.
