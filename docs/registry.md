...
Curated bootstrap records also license [source-scoped definitions and summons](definitions.md). Closed form, prefix, boundary, and date-requirement facets are consumed by those analyses. Source identities and locations remain attached to each assertion; unavailable constructor execution is distinct from a verified declaration signature. The independent manifests include LaTeX kernel/package sources, TeX primitive documentation, and paired-delimiter documentation. Only language facts were transcribed; no implementation code from these sources is shipped.
