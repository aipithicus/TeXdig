...
} from "../../packages/texdig/dist/latex/sublanguages/checkpoints.js";
import { SourceSnapshot } from "../../packages/texdig/dist/source/index.js";
import type { ArgumentPattern, ArgumentToken } from "../../packages/texdig/src/registry/types.ts";
...
export function normalizeArgspec(spelling: string): readonly ArgumentPattern[] {
  const result = parseArgspec(
    new SourceSnapshot(new TextEncoder().encode(spelling), {
      sourceId: "registry-argspec",
      revision: 0,
    }),
    { strict: true },
  );
